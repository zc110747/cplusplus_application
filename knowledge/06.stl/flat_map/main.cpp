...
// 辅助模板,检测是否为std::flat_map
template<typename T, typename = std::enable_if_t<std::is_same<T, std::flat_map<typename T::key_type, typename T::mapped_type>>::value>>
...
void algorithm_process(void)
{
    ...
    std::flat_map<int, int> vcon_4 = {
        {1, 1},
        {3, 3}, 
        {5, 5},
    };
    std::flat_map<int, int> vcon_5 = {
        {2, 2},
        {4, 4},
        {6, 6},
        {8, 8},
    };
    std::flat_map<int, int> vcon_6;
...
{
    std::flat_map<std::string, int> usermap = {
        {"alpha", 4},
        {"beta", 2}, 
        {"gamma", 3},   
    };
    ...
    std::flat_map<std::string, int>::iterator iterbegin = usermap.begin();
    std::flat_map<std::string, int>::iterator iterend = usermap.end();
    std::flat_map<std::string, int>::const_iterator iterconstbegin = usermap.cbegin();
    std::flat_map<std::string, int>::const_iterator iterconstend = usermap.cend();
    std::flat_map<std::string, int>::reverse_iterator iterrbegin = usermap.rbegin();
    std::flat_map<std::string, int>::reverse_iterator iterrend = usermap.rend();
    std::flat_map<std::string, int>::const_reverse_iterator iterconstrbegin = usermap.crbegin();
    std::flat_map<std::string, int>::const_reverse_iterator iterconstrend = usermap.crend();
    ...
    std::pair<std::flat_map<std::string, int>::iterator, std::flat_map<std::string, int>::iterator> ret;
    ...
    std::flat_map<std::string, int>::iterator lower = usermap.lower_bound("beta");
    std::flat_map<std::string, int>::iterator upper = usermap.upper_bound("beta");
    ...
    //find, erase
    std::flat_map<std::string, int>::iterator it = usermap.find("gamma");
    ...
    std::flat_map<std::string, int> swapmap;
    ...
    // key compare
    std::flat_map<std::string, int, compare_by_length> compare_map = {
        {"alpha1", 3},
        {"beta2", 4},
        {"gamma3", 1},
    };
    std::cout<<"compare_map: ";
    for(const auto &x:compare_map) {
        std::cout<<x.first<<"=>"<<x.second<<" ";
    }
    std::cout<<std::endl;
