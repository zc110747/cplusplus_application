...
void algorithm_process(void)
{
    ...
    std::unordered_multimap<int, int> vcon_0 = {
        {1, 1},
        {3, 3},
        {4, 4},
        {5, 5},
    ...
    std::unordered_multimap<int, int> vcon_1 = {
        {5, 5},
        {4, 4},
    };
    ...
    // 变换算法
    std::unordered_multimap<int, int> vcon_new;
    ...
    // // 生成算法
    std::unordered_multimap<int, int> vcon_2;
    ...
    std::unordered_multimap<int, int> vcon_4 = {
        {1, 1},
        {3, 3}, 
        {5, 5},
    };
    std::unordered_multimap<int, int> vcon_5 = {
        {2, 2},
        {4, 4},
        {6, 6},
        {8, 8},
    };
    std::unordered_multimap<int, int> vcon_6;
...
{
    std::unordered_multimap<std::string, int> ucon_0 = {
        {"alpha", 0},
        {"beta", 0}, 
        {"gamma", 1},   
    };
    ...
    show_container(ucon_1, "ucon_1");
    std::cout << "size:" << ucon_1.size() << std::endl;
    ucon_1.clear();
    std::cout << "clear size:" << ucon_1.size() << std::endl;
    std::cout << "max_size:" << ucon_1.max_size() << std::endl;
    std::cout << "empty:" << ucon_1.empty() << std::endl;
    std::cout << "load_factor" << ucon_1.load_factor() << std::endl;
    std::cout << "max_bucket_count" << ucon_1.max_bucket_count() << std::endl;
    std::cout << "max_load_factor" << ucon_1.max_load_factor() << std::endl;
    show_container(ucon_0, "ucon_0");

    //bucket, bucket_count, bucket_size
    std::cout << "bucket:" << ucon_0.bucket("beta") << std::endl;
    auto bucket_count = ucon_0.bucket_count();
    std::cout << "bucket count:" << bucket_count << std::endl;
    for(auto i=0; i < bucket_count; i++)
    {
        std::cout << "bucket #" << i<<" has " << ucon_0.bucket_size(i)<<" elements." << std::endl;
    }

    //begin, end, cbegin, cend
    std::unordered_multimap<std::string, int>::iterator iterbegin = ucon_0.begin();
    std::unordered_multimap<std::string, int>::iterator iterend = ucon_0.end();
    std::unordered_multimap<std::string, int>::const_iterator iterconstbegin = ucon_0.cbegin();
    std::unordered_multimap<std::string, int>::const_iterator iterconstend = ucon_0.cend();
    std::cout << "begin: " << iterbegin->first<<"=>" << iterbegin->second<<",";
    std::cout << "cbegin: " << iterconstbegin->first<<"=>" << iterconstbegin->second<<",";

    //count
    std::cout << "count:" << ucon_0.count("beta") << std::endl;

    //emplace, emplace_hint
    ucon_0.emplace("delta", 6);
    show_container(ucon_0, "emplace");
    ucon_0.emplace_hint(ucon_0.begin(), "epsilon", 10);
    show_container(ucon_0, "emplace");
    ...
    ucon_0.erase(it);
    show_container(ucon_0, "erase");

    //get_alloctator
    std::pair<const std::string, int>* p;
    p = ucon_0.get_allocator().allocate(5);
    std::cout << "size:" << sizeof(p)<<"," << sizeof(std::unordered_multimap<std::string, int>::value_type) << std::endl;
    ucon_0.get_allocator().deallocate(p, 5);

    //insert
    ucon_0.insert(ucon_0.begin(), std::pair<std::string, int>("gamma", 5));
    ucon_0.insert(ucon_0.find("delta"), std::pair<std::string, int>("zeta", 9));
    show_container(ucon_0, "insert");
    ucon_0.insert({{"delta", 6}, {"epsilon", 9}});
    show_container(ucon_0, "insert");

    //hash_function
    typedef std::unordered_multimap<std::string, int> stringintMap;
    stringintMap::hasher hash_fn = ucon_0.hash_function();
    std::cout << "file hash value:" << hash_fn("file") << std::endl;

    //key_eq
    bool equal = ucon_0.key_eq()("this", "This");
    std::cout << "key_eq:" << equal << std::endl;

    std::unordered_multimap<std::string, int> swapunordered_multimap;

    //swap
    swapunordered_multimap.swap(ucon_0);
    show_container(swapunordered_multimap, "swap");

    //rehash, reserve
    swapunordered_multimap.rehash(15);
    std::cout << "rehash:" << swapunordered_multimap.bucket_count() << std::endl;
    swapunordered_multimap.reserve(5);
    std::cout << "reserve:" << swapunordered_multimap.bucket_count() << std::endl;
