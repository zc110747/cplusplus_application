...
1. 重写(overrider)是指在派生类中重新定义基类的虚函数，这样调用派生类时重写基类的函数。
2. 重载(overload)是指在同一个作用域内定义多个同名函数，它们的参数列表不同。
3. 隐藏(overhide)是指在派生类中定义一个与基类同名的函数(基类不带virtual)，会隐藏基类中相同名称的函数，不能够直接调用。
...
列表初始化是一种在C++中初始化对象的方法，它允许你使用花括号{}来初始化对象的成员。对于C++库中的对象，如vector，map等，已经实现了列表初始化。对于自定义的class类型，可以引入std::initializer_list作为参数的构造函数，支持列表初始化；具体示例如下所示。
...
注1: 类也支持在构造函数中进行初始化，会在立即初始化后执行，覆盖相应数据。
注2: 类对象的初始化顺序为"立即初始化 => 成员初始化列表 => 构造函数调用", 最后结果以最后一步的结果为准。
...
1. 类的静态数据成员如果是const类型，可以直接立即初始化
2. 类的静态数据成员如果是非const类型，可以使用inline进行立即初始化
...
任何非引用类型都可以成为联合体的数据成员，这样的联合体即所谓的非受限联合体。
