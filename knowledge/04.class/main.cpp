...
3. 多态，虚函数，纯虚函数、接口类
多态
它允许不同的对象通过相同的接口来表现出不同的行为。在C++中，多态主要通过虚函数和继承来实现，核心思想是：同一个接口，不同的实现

静态多态（编译时多态）
静态多态是在编译阶段就确定要调用的函数，主要通过函数重载和运算符重载来实现

动态多态（运行时多态）
动态多态是在运行时根据对象的实际类型来决定调用哪个函数，主要通过虚函数和继承来实现。
...
虚函数表
每个包含虚函数的类都会有一个对应的虚函数表，这个表是一个存储类成员函数指针的数组。每个拥有虚函数的对象都会有一个指向该类虚函数表的指针（通常被叫做虚表指针，vptr），虚函数表(vtable)在编译时生成，在运行时对象会生成虚函数指针(vptr)指向虚函数表。
