...
- 构造函数和析构函数均为默认。不能存在用户定义的构造函数，包括构造函数、拷贝构造函数、移动构造函数和析构函数。
- 赋值和移动运算符均为默认。不能存在用户自定义的赋值运算符和移动运算符。
- 没有虚函数和虚基类，该类型不能包含虚函数表或虚继承。
- 基类和非静态成员都是平凡类型：该类型的基类和非静态成员变量也必须是平凡类型。
...
- 所有非静态成员变量具有相同的访问权限控制。所有的成员变量必须都是public、protected或private。
- 在类或结构体继承时，满足以下两种情况之一。 
  - 派生类中有非静态成员变量，且只有一个仅包含静态成员(无成员)的基类。
  - 基类有非静态成员，而派生类没有非静态成员。
- 类中的第一个非静态成员的类型与其基类和本身类型不同。
- 没有虚函数和虚继承；类型不能包含虚函数表或虚继承机制。
...
- 平凡类型可以通过简单的内存拷贝进行复制，并且在复制过程中不会有任何特殊行为；但不一定是标准布局类型。因为它可能有不同的访问控制或其他不符合标准布局类型要求的特性。
...
- 满足平凡类型（trivial）：类型的所有构造函数、析构函数、赋值运算符都是默认的，没有用户自定义的特殊行为。
- 满足标准布局类型（standard-layout）：类型的内存布局是标准的、可预测的，它保证了类型在不同平台上的ABI兼容。
- 所有非静态成员都是POD类型。
