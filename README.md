...
- version: C++20引入的版本类，用于表示C++标准的库定义。
...
- 显式模板实例化（Explicit Template Instantiation）是C++中的一个特性，它允许程序员在一个编译单元中显式地实例化一个模板。
- 外部模板（Extern Template）是C++中的一个特性，它允许程序员在一个编译单元中显式地实例化一个模板，而在其他编译单元中使用这个实例化的模板，而不需要再次实例化，主要应用与模板函数中。
...
- bit: C++20引入的位操作类，用于对二进制位进行操作。
  ...
  - bit_cast：是C++20引入的一个函数模板，用于在不同类型的对象之间进行位级转换。
...
- 三向比较运算符：<=>，C++20引入的运算符，用于比较两个对象的大小关系。另外定义在compare中，支持std::strong_ordering、std::weak_ordering和std::partial_ordering。
- C++26新增功能(目前不一定支持)
