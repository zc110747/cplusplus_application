...
本系列中大部分例程都是单项目文件，可以直接在线运行。另外测试使用版本是g++ 9.5.0或更新，早期版本可能编译报错，一般为不支持最新特性；建议在如下环境测试。
...
详细文档: [支持C语法说明](./knowledge/01.c_style/README.md)
...
- 模板类型分为函数模板，类模板，变量模板，别名模板(using声明)。
...
- 聚合类型（Aggregate Type）指的是那些可以通过简单的初始化列表进行初始化的类型。
